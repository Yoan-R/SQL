...
SELECT id_livre, titre FROM livre WHERE id_livre IN (SELECT id_livre from emprunt WHERE date_rendu IS null);
...
SELECT COUNT(*) AS 'nombre de livre' FROM emprunt WHERE id_abonne = (SELECT id_abonne FROM abonne WHERE prenom = 'Guillaume');
...
SELECT prenom FROM abonne WHERE id_abonne IN (SELECT id_abonne FROM emprunt WHERE id_abonne IN (SELECT));
SELECT prenom FROM abonne WHERE id_abonne IN (SELECT id_abonne FROM emprunt WHERE id_abonne IN (SELECT id_livre FROM livre WHERE auteur = 'ALPHONSE DAUDET'));
  +--------+
  | prenom |
  +--------+
  | laura  |
